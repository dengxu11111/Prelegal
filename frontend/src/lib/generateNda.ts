...
export function generateCoverPage(data: NdaFormData): string {
  const mndaTerm =
    data.mndaTermType === "expires"
      ? `- [x] Expires ${blank(data.mndaTermYears, "___")} year(s) from Effective Date.\n- [ ] Continues until terminated in accordance with the terms of the MNDA.`
      : `- [ ] Expires ___ year(s) from Effective Date.\n- [x] Continues until terminated in accordance with the terms of the MNDA.`;

  const confTerm =
    data.confidentialityTermType === "years"
      ? `- [x] ${blank(data.confidentialityTermYears, "___")} year(s) from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws.\n- [ ] In perpetuity.`
...
${data.modifications.trim() ? `### MNDA Modifications\n\n${data.modifications.trim()}` : ""}
...
| | PARTY 1 | PARTY 2 |
|:---|:---:|:---:|
| **Print Name** | ${blank(data.party1.name, "___________")} | ${blank(data.party2.name, "___________")} |
| **Title** | ${blank(data.party1.title, "___________")} | ${blank(data.party2.title, "___________")} |
| **Company** | ${blank(data.party1.company, "___________")} | ${blank(data.party2.company, "___________")} |
| **Notice Address** | ${blank(data.party1.noticeAddress, "___________")} | ${blank(data.party2.noticeAddress, "___________")} |
...
export function generateStandardTerms(data: NdaFormData): string {
  const govLaw = blank(data.governingLaw, "___________");
  const jurisdiction = blank(data.jurisdiction, "___________");
